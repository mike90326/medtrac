...
public class ManualRetryScheduler
{
  private static final Logger LOGGER = Logger.getLogger(ManualRetryScheduler.class.getName());
  ...
  public void SetNewSchedule(ScheduleData retry) throws InterruptedException, IOException {
    ...
    ScheduledFuture<?> sfHandle = this.rScheduler.scheduleAtFixedRate(
      new ManualSendtoDev(retry), startTime / 1000L, interval * 60L, TimeUnit.SECONDS);
    ManualSendtoDev.setSchedule(sfHandle, retry);
    Runnable endSchedule = new EndSchedule(sfHandle);
    this.rScheduler.schedule(endSchedule, endTime / 1000L, TimeUnit.SECONDS);

    LOGGER.log(Level.INFO, "(ManualRetryScheduler)Schedule set for BoxID: {0}", boxid);
  }
  private final class EndSchedule implements Runnable {
    ...
    public void run()
    {
      ...
      ManualSendtoDev.scheduleList.remove(this.rSchedFuture);

      if (ManualSendtoDev.scheduleList.isEmpty()) {
        System.out.println("Shutting down Retry Scheduler due to no pending task");
        ManualRetryScheduler.this.rScheduler.shutdown();
      }
    }
  }
}